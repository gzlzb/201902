...
def q_squared(data, response):
    ...
    predicted = cross_val_predict(lr, X, y, cv = 40)
...
def ols(data1, response):
    ...
    data1['pre'] = predicted
    data1.eval('res=abs(LogBIO-pre)',inplace=True)
    data2 = data1.sort_values('res',ascending=True)
    data3 = data2.drop(['pre','res'], axis=1)
    data4 = data3[int(n/20):n]
    y2 = data4.LogBIO
    X2 = data4.drop([response], axis=1)
    """cv = pd.DataFrame({'y':y, 'y0':predicted0})
    cv = pd.DataFrame({'y':y, 'y1':predicted, 'y0':predicted0})"""
    #cv = pd.DataFrame({'y':y, 'y1':predicted})
    ols = smf.OLS(y2,X2).fit()
    """r2 = smf.ols('y~y1', cv).fit().rsquared
    radj_2 = smf.ols('y~y0',cv).fit().rsquared_adj"""
...
def forward_selected(data, pre, response):
    ...
    while remaining and current_score == best_new_score:
        ...
        if current_score < best_new_score:
            ...
            print(20000-current_score)
            #print(score)
            cor_feature = cv.drop(['pre','res'], axis=1)
            cor_feature.to_csv("correct_2.csv", index=False)
